...
import { MinusCircle, X, ChevronDown, Loader2, Info } from 'lucide-react';
...
export default function StockOutModal({ isOpen, onClose, preselectedItemId, onSuccess }: StockOutModalProps) {
  ...
  useEffect(() => {
    ...
    async function loadData() {
      ...
      try {
        ...
        console.error('Failed to load data for StockOutModal', err);
      } finally {
  ...
  const onSubmit = async (data: StockOutFormValues) => {
    if (!user) return;
    if (selectedItemDetails && data.quantity > selectedItemDetails.current_stock) {
      setErrorMessage(t(
        `Cannot sell ${data.quantity} units. Only ${selectedItemDetails.current_stock} units available in stock.`,
        `${data.quantity} ইউনিট বিক্রি করা সম্ভব নয়। স্টকে মাত্র ${selectedItemDetails.current_stock} ইউনিট উপলব্ধ রয়েছে।`
      ));
      return;
    }
    ...
    setErrorMessage(null);
    try {
      await recordSale(data.itemId, data.quantity, user.id, data.notes);
      ...
      <div className="bg-white border border-slate-200 w-full max-w-md rounded-2xl shadow-xl overflow-hidden animate-fade-in">
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-slate-100 bg-slate-50">
          <div className="flex items-center gap-2">
            <MinusCircle className="w-5 h-5 text-emerald-650" />
            <h3 className="font-bold text-slate-800 text-lg">{t('Sales Entry Form', 'স্টক আউট এন্ট্রি ফর্ম')}</h3>
          </div>
          <button
            onClick={onClose}
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="p-6 space-y-5">
          ...
          <div>
            <label className="block text-xs font-semibold uppercase tracking-wider text-slate-550 mb-1.5">
              {t('Customer Info & Notes', 'গ্রাহকের বিবরণ ও মন্তব্য')}
            </label>
            <textarea
              {...register('notes')}
              placeholder={t('e.g. Customer: Md. Rahim, Phone: 01712xxxxxx, Bill No...', 'যেমন: কাস্টমার - মো: রহিম, মোবাইল - ০১৭১২xxxxxx, বিল নং ইত্যাদি')}
          ...
          <div className="flex gap-3 pt-2">
            ...
            >
              ...
              <span>{t('Confirm Sale', 'বিক্রয় নিশ্চিত করুন')}</span>
            </button>
          </div>
        </form>
      </div>
